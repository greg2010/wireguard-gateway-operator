...
echo "    ships with credentials.source=Secret (the on-prem gateway has no GCE"
echo "    metadata server for keyless identity), so deliver the key via your"
echo "    cluster's declarative secret mechanism (External Secrets Operator /"
echo "    SealedSecrets / GitOps), not this file."
